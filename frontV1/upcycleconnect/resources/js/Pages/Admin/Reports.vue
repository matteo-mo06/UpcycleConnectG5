        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-primary">
              ...
              <th class="text-left text-white font-medium px-5 py-3 hidden md:table-cell">Motif</th>
              <th class="text-left text-white font-medium px-5 py-3 hidden lg:table-cell">Signalé par</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-5 py-3 text-gray-500 text-xs hidden lg:table-cell">{{ report.reporter }}</td>

              <td class="px-5 py-3 text-gray-500 text-xs hidden lg:table-cell">{{ report.date }}</td>
              ...
              <td class="px-5 py-3">
                <div class="flex items-center justify-end gap-1">
                  ...
                  <button
                    v-if="report.status === 'À traiter' || report.status === 'En cours'"
    ...
    >
      ...
      <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
        ...
        <div class="px-6 py-5 space-y-5">
          ...
          <div v-if="detailReport.status === 'À traiter' || detailReport.status === 'En cours'">
            <p class="text-xs text-gray-400 mb-2">Action sur le contenu</p>
            <div class="flex flex-wrap gap-2">
              <button
                @click="warnUser(detailReport)"
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex justify-between items-center">
          <button
            v-if="detailReport.status === 'À traiter' || detailReport.status === 'En cours'"
...
const reports = ref([
  {
    id: 1,
    type: 'Annonce',
    contentTitle: 'Lot composants électroniques',
    contentAuthor: 'Marc Artisan',
    contentPreview: 'Résistances, condensateurs, LED et autres composants issus de démontage. Lot de 500+ pièces. 15 €.',
    reporter: 'Lucie Bernard',
    reporterEmail: 'lucie.b@gmail.com',
    reason: 'Contenu inapproprié',
    date: '08/03/2026',
    status: 'À traiter',
    message: 'Cette annonce propose des composants dont certains semblent neufs et emballés. Je doute que ce soit de la récupération.',
  },
  {
    id: 2,
    type: 'Utilisateur',
    contentTitle: 'Profil de ProSpam99',
    contentAuthor: 'ProSpam99',
    contentPreview: 'Compte créé le 01/03/2026 — 12 annonces publiées en 48h, toutes identiques.',
    reporter: 'Camille Dupont',
    reporterEmail: 'camille.dupont@mail.com',
    reason: 'Spam',
    date: '07/03/2026',
    status: 'En cours',
    message: 'Cet utilisateur publie la même annonce en masse depuis 2 jours. Clairement du spam.',
  },
  {
    id: 3,
    type: 'Annonce',
    contentTitle: 'Vente iPhone 14 neuf',
    contentAuthor: 'Thomas Salarié',
    contentPreview: 'iPhone 14 128 Go, neuf sous blister, jamais ouvert. 650 €.',
    reporter: 'Sophie Martin',
    reporterEmail: 'sophie.m@mail.com',
    reason: 'Hors charte',
    date: '06/03/2026',
    status: 'À traiter',
    message: 'Cette annonce vend un produit neuf. Ça n\'a rien à voir avec le recyclage ou l\'upcycling.',
  },
  {
    id: 4,
    type: 'Annonce',
    contentTitle: 'Recherche palette bois urgente',
    contentAuthor: 'Thomas Salarié',
    contentPreview: 'Recherche 2-3 palettes en bon état pour projet de mobilier extérieur.',
    reporter: 'Marc Artisan',
    reporterEmail: 'marc.artisan@pro.fr',
    reason: 'Doublon',
    date: '04/03/2026',
    status: 'Résolu',
    message: 'Même demande publiée 3 fois en 2 jours par le même utilisateur.',
  },
  {
    id: 5,
    type: 'Utilisateur',
    contentTitle: 'Profil de GrossierX',
    contentAuthor: 'GrossierX',
    contentPreview: 'Compte avec comportement agressif dans les échanges de messages.',
    reporter: 'Lucie Bernard',
    reporterEmail: 'lucie.b@gmail.com',
    reason: 'Comportement abusif',
    date: '01/03/2026',
    status: 'Ignoré',
    message: 'Cet utilisateur m\'a envoyé des messages très agressifs après que j\'ai refusé son offre d\'échange.',
  },
])
...
const stats = computed(() => [
  ...
  {
    label: 'À traiter',
    value: reports.value.filter(r => r.status === 'À traiter').length,
    color: '#ef4444',
  ...
  {
    label: 'En cours',
    value: reports.value.filter(r => r.status === 'En cours').length,
    color: '#f59e0b',
  ...
  {
    label: 'Résolus',
    value: reports.value.filter(r => r.status === 'Résolu').length,
    color: '#8dc734',
...
function openDetail(report) {
  ...
  if (target && target.status === 'À traiter') target.status = 'En cours'
  detailReport.value = target ?? report
}

function ignoreReport(report) {
  const target = reports.value.find(r => r.id === report.id)
  if (target) target.status = 'Ignoré'
}

function resolveReport(report) {
  const target = reports.value.find(r => r.id === report.id)
  if (target) target.status = 'Résolu'
}

function warnUser(report) {
  alert(`Avertissement envoyé à l'auteur "${report.contentAuthor}".`)
}

function removeContent(report) {
  if (!confirm(`Supprimer le contenu "${report.contentTitle}" et résoudre le signalement ?`)) return
  const target = reports.value.find(r => r.id === report.id)
  if (target) target.status = 'Résolu'
