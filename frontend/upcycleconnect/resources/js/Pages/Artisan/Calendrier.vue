<template>
    <ArtisanLayout>
        ...
        <div class="mb-4">
            ...
            <p class="text-sm text-gray-500 mt-1">Vos événements, formations et projets créés</p>
        </div>
...
const VIEW_ROUTES = {
    event:     '/artisan/evenements',
    formation: '/artisan/formations',
    project:   '/artisan/projets',
}
...
const calendarOptions = computed(() => {
    ...
    return {
        ...
        eventClick: (info) => {
            ...
            const id = rawId.replace(/^(event|formation|project)-/, '')
            selectedItem.value = {
                id,
...
onMounted(async () => {
    ...
    const [evRes, foRes, prRes] = await Promise.all([
        api.get('/user/my-events', { params }).catch(() => ({ data: { data: [] } })),
        api.get('/user/my-formations', { params }).catch(() => ({ data: { data: [] } })),
        api.get('/user/my-projects', { params }).catch(() => ({ data: { data: [] } })),
    ])
    events.value     = evRes.data?.data ?? []
    formations.value = foRes.data?.data ?? []
