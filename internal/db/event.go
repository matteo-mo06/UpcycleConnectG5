...
func CreateEvent(e models.Event) error {
	_, err := config.Conn.Exec(`
		INSERT INTO EVENT (id_event, title_event, description_event, date_event, location_event, capacity, price_cents)
		VALUES (?, ?, ?, ?, ?, ?, ?)`, e.Id_event, e.Title_event, e.Description_event, e.Date_event, e.Location_event,
		e.Capacity, e.Price_cents,
	)
	return err
}
...
func UpdateEvent(e models.Event) error {
	_, err := config.Conn.Exec(`
		UPDATE EVENT SET title_event = ?, description_event = ?, date_event = ?, location_event = ?,
		capacity = ?, price_cents = ?
		WHERE id_event = ?`, e.Title_event, e.Description_event, e.Date_event, e.Location_event,
		e.Capacity, e.Price_cents, e.Id_event,
	)
	return err
}
