...
func GetUserById(id string) (models.User, error) {
	...
	row := config.Conn.QueryRow(`SELECT id_user, email, password_user, first_name, last_name, upcycling_score, premium
		FROM USER
		WHERE id_user = ?`, id)
...
func UpdateUser(user models.User) error {
	_, err := config.Conn.Exec(`
		UPDATE USER SET
			email = ?, first_name = ?, last_name = ?, upcycling_score = ?, premium = ?
		WHERE id_user = ?`,
		user.Email, user.First_name, user.Last_name, user.Upcycling_score, user.Premium, user.Id_user,
	)
	return err
}
